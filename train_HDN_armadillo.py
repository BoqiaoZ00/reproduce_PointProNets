...
class RealPatchHeightmapDataset(Dataset):
    ...
    def __init__(
        ...
        self.norm_meshes = [self.norm_meshes[1]] # only use armadillo for simple runs
    ...
    def __getitem__(self, idx: int):
        ...
        heightmaps: List[torch.Tensor] = HGN.project_points_to_heightmap_test(
            [pverts_cpu], [patch_normal], r=self.r, k=self.k
        )
        if len(heightmaps) == 0 or heightmaps[0] is None:
            raise RuntimeError("Projection returned no heightmap.")
        clean = heightmaps[0].to(torch.float32)  # (k, k) expected
        if clean.ndim != 2:
            print("Projector returned heightmap with ndim != 2")
            # if projector returns something else, force reshape
            clean = clean.view(self.k, self.k)

        # Normalize clean heightmap to a stable range similar to your synthetic version
        ...
        # Create "noisy" counterpart to match your original training interface (optional)
        valid_mask = (clean != 0)
        noisy, noise_pts = self._add_sparse_point_noise(
            clean, valid_mask, num_points=30, rel_mag=0.25, clamp=1.5, seed=0
        )

        # Match your return signature & shapes
        return (
            clean.unsqueeze(0).to(torch.float32),                       # (1, k, k)
            noisy.unsqueeze(0).to(torch.float32),                       # (1, k, k)
        )

    # ---------- helpers -----------
    def _add_sparse_point_noise(
            self,
            clean: torch.Tensor,  # (H, W) float32
            valid_mask: torch.Tensor,  # (H, W) bool  (True = valid height)
            num_points: int = 30,
            rel_mag: float = 0.25,  # ~25% of valid dynamic range per spike
            clamp: float = 1.5,
            seed: int = 123,
    ):
        """
        Adds 'num_points' obvious spikes only on valid pixels.
        Returns (noisy, noise_coords_mask).
        """
        H, W = clean.shape
        assert H == 32 and W == 32, "This helper is tuned for 32x32; adjust if needed."

        g = torch.Generator(device=clean.device)
        if seed is not None:
            g.manual_seed(seed)

        # robust amplitude from valid pixels
        vals = clean[valid_mask]
        if vals.numel() == 0:
            return clean.clone(), torch.zeros_like(valid_mask)

        lo, hi = torch.quantile(vals, torch.tensor([0.02, 0.98], device=vals.device))
        amp = (hi - lo).clamp(min=1e-6)

        # candidate indices (only valid)
        valid_idx = torch.nonzero(valid_mask, as_tuple=False)  # (N, 2)
        N = valid_idx.size(0)
        take = min(num_points, N)

        # sample without replacement
        perm = torch.randperm(N, generator=g, device=clean.device)[:take]
        coords = valid_idx[perm]  # (take, 2)

        # make “obvious” spikes: +/- rel_mag * amp with a bit of randomness
        rand_scale = 0.7 + 0.6 * torch.rand(take, device=clean.device)  # ~[0.7, 1.3]
        signs = torch.where(torch.rand(take, device=clean.device) < 0.5, -1.0, 1.0)
        delta = signs * (rel_mag * amp * rand_scale)

        noisy = clean.clone()
        noisy[coords[:, 0], coords[:, 1]] = (noisy[coords[:, 0], coords[:, 1]] + delta)
        noisy = noisy.clamp(-clamp, clamp)

        # mask of where we injected noise (useful for visualization/debug)
        noise_mask = torch.zeros_like(valid_mask)
        noise_mask[coords[:, 0], coords[:, 1]] = True
...
def run_denoising_training(
    ...
    train_ds = RealPatchHeightmapDataset(
        ...
        r=0.08,            # projection radius
        num_patches_per_mesh=1000,
        patch_radius=0.1,
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints/denoising/advanced-10-168/armadillo",
